...
// RegisterInterfaceFromArgsFunc initializes a new scheduler interface from given arguments
type RegisterInterfaceFromArgsFunc = func(ctx context.Context, logger *logrus.Logger) (Interface, error)

// RegisterFlagsFunc adds necessary flags from a specific scheduler to the overall command.
...
// Registrations holds a map of scheduler names and their corresponding Registration implementation.
type Registrations map[string]*Registration

// Register should be implemented by the scheduler and add themselves to the registrations.
